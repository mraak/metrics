"""
Compute region_metrics + territory_metrics: per-entity × per-brand × per-period
analytical metrics, built by ONE pipeline run at two grains.

Grains
  region_metrics    – year_month × period_type × brand_name × region_name
  territory_metrics – year_month × period_type × brand_name × territory_name
                      (regions summed; market share recomputed from summed own
                       vs summed franchise totals — share is NOT additive)
...
# ── 4. Brand-level national FCST growth_py ───────────────────────────────────
...
# ── 5. The metric pipeline, parameterized by grain ────────────────────────────
# One pipeline, run per entity level (region, territory). `monthly` is one row
# per year_month × brand × entity with own_u/own_s/tot_u/tot_s; everything else
# (period windows, growth, deviations, ranks) derives from those four sums.
...
def build_metrics(monthly, entity_col, id_cols):
    """Period aggregates → growth → deviations → ranks at one grain.

    entity_col: the dimension the time series runs over ("region_name" /
    "territory_name"). id_cols: identity columns kept on every output row.
    """
    base = monthly.copy()
    ...
    # PY: shift 12 within brand-entity
    ...
    ID = ["year_month", "brand_name", "franchise"] + id_cols
    ...
    for period_type, (cu, cs, ctu, cts, cspy, cspp, cupy, cupp, cfg, cfg_u) in PERIODS.items():
        ...
        # Growth deviation: entity YoY (PY) growth minus the brand's VOLUME-WEIGHTED
        # national YoY growth (per brand × year_month). Peer-relative growth momentum,
        # the companion to mshare_deviation. Summing over either grain gives the same
        # national reference, so region and territory deviations are mutually consistent.
        ...
        # Market share deviation: entity market_share minus the brand's VOLUME-WEIGHTED
        ...
        sub["mshare_dev_pp"] = grp_be["mshare_deviation"].transform(lambda x: x.shift(shift_pp))
    ...
    # Ranks: NTILE(100), ranked within year_month × period_type × brand
    def ntile100(s):
...
# ── 6. Run at both grains ─────────────────────────────────────────────────────

metrics = build_metrics(
    base, "region_name", ["region_name", "territory_id", "territory_name"])

# Territory grain: sum the monthly own/total sums over each territory's regions.
# Market share is NOT additive — it re-derives inside the pipeline from the
# summed own vs summed franchise totals.
t_monthly = (
    base.groupby(["year_month", "brand_name", "franchise", "territory_id", "territory_name"],
