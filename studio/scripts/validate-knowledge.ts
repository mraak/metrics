...
// validates knowledge_definitions.json against the live metrics.db schema.
...
function tableColumns(db: Database.Database, table: string): Set<string> {
  return new Set((db.pragma(`table_info(${table})`) as { name: string }[]).map(r => r.name))
}
...
for (const s of Object.values(signals)) {
  ...
  if (!(t in colsByTable)) colsByTable[t] = tableColumns(db, t)
}
